...
2. 选择已保存流程，或直接录制新流程。
3. 点击 `录制`。
4. 执行希望 Remember 捕获的安全本地操作。
5. 点击 `停止`。
6. 工作台会打开并显示录制得到的步骤。
7. 按需检查和编辑步骤。
8. 点击 `保存流程` 或 `另存为`。
9. 点击 `运行` 或 `重放` 开始回放。
10. 回放期间可随时按 `Ctrl + Alt + S` 急停。

## 支持的输入

Remember 会记录普通文本、鼠标操作、修饰键快捷键和常见控制键。拖拽会保存采样轨迹点，
用于回放更接近真实的拖动路径；未按住鼠标的普通移动仍不会单独保存。比如 `Ctrl + F10`
...
- 输入类回放步骤需要目标窗口元数据。Remember 会先尝试切回录制目标窗口；目标缺失、无法切换或切换后仍明显不一致时会安全停止，而不是继续点击或输入。
